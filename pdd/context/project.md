...
- **Phase B0**: Real preview backend — run the crew's built app in the Cabin. The deployment layer (`InProcessDeploymentBackend`) was a STUB (synthetic `http://preview.voyage-xxxx.local` URL, ran nothing); preview now launches the app as a **long-running process inside the user's Cabin** (Phase 0b), captures a **real reachable URL + tail logs**, and **reaps it on a hard lifetime cap** (start/status/logs-tail/stop — STREAMING is B1, embedded iframe B2, PTY terminal B3). Built as a **standalone `PreviewService` over `CabinService`**, leaving the pipeline's `InProcessDeploymentBackend` byte-identical. The Cabin contract gained long-running-service support: secret-free `ServiceHandle{service_id, url, port, status}` + `ServiceStatus` and four `CabinBackend` methods — `start_service(user_id, command, *, env=None, port=None)`, `service_logs(..., tail=200)`, `service_status`, `stop_service`. `NullCabinBackend` (CI default) is deterministic in-memory (stable `http://127.0.0.1:<port>`, canned logs, running→stopped; command/env never stored); `GVisorCabinBackend` (opt-in, lazy `aiodocker`) launches the process detached in the persistent Cabin, tees stdout/stderr to a per-service log file, exposes the bound localhost port — egress stays deny-by-default + allow-list, command/env never logged. `app/services/preview_service.py` (`PreviewService(cabin_service, settings)` + `PreviewError`) keeps a process-local per-user registry (v1 single-worker): `start` (ensure Cabin → resolve command (arg › `preview_default_command`) → start → stamp hard max lifetime → `PreviewInfo{preview_id, url, status}`, ONE per user), `status`/`logs`/`stop` (idempotent), `reap_expired(*, now=None)` (stop past-lifetime, injectable now), `stop_all` (shutdown). `app/main.py` constructs `app.state.preview_service` and folds `reap_expired` into the existing Cabin reaper loop + `stop_all` on shutdown. API (default-deny, owner-scoped via `get_authorized_voyage`): `POST`/`GET`/`DELETE /api/v1/voyages/{id}/preview` + `GET .../preview/logs?tail=`. `PreviewInfo` carries NO secret. New settings `preview_default_command` (generic dev-server command; real per-app detection is future), `preview_max_lifetime_seconds=1800`, `preview_idle_timeout_seconds=900`. No migration (in-memory registry), no new dependency (`aiodocker` stays lazy — app imports without Docker).
- **Phases B1 + B2**: the **Preview panel** — live log streaming + the embedded running app. **B1** adds an SSE log stream `GET /api/v1/voyages/{id}/preview/logs/stream` (owner-scoped via `get_authorized_voyage`) — a `StreamingResponse(text/event-stream)` that polls `PreviewService.logs(user_id, tail=500)` on a short interval (`_LOG_POLL_INTERVAL_S`, patchable), emits only the NEW delta as `data: <line>\n\n` (longest-common-prefix anchor so a rotated tail re-syncs), and self-terminates on client disconnect (`request.is_disconnected()`), when the preview is stopped/absent, or at a hard iteration cap — mirroring the pipeline SSE shape; app stdout/stderr only, never a secret. **B2** adds the frontend surface: a new top-level **Preview** deck view (`app/app/preview/page.tsx` + a `{href:"/app/preview", label:"Preview"}` tab in `DeckShell`) rendering `components/observation-deck/PreviewPanel.tsx` — a **Start preview** button (none running) / status + **Stop** (running), and when running a **sandboxed iframe** of `PreviewInfo.url` (`sandbox="allow-scripts allow-same-origin allow-forms allow-popups"` + an "open in new tab" link) alongside a **live logs pane** (follow/pause/clear). Data via `lib/preview.ts` (`PreviewInfo` type + `startPreview`/`getPreview`/`stopPreview`), `hooks/usePreview.ts` (React Query status — 404→`null` empty state — + start/stop mutations that invalidate the status query), and `hooks/usePreviewLogs.ts` (reads the SSE stream via fetch + a ReadableStream reader with the bearer token, like `useVoyageStream`'s SSE fallback; capped ring buffer; aborts on unmount/disable). **CSP**: `next.config.mjs` gained `frame-src 'self' http://localhost:* http://127.0.0.1:* https:` so the iframe loads the Cabin-run preview origin. No backend service change (consumes B0), no new dependency, no migration.
- **Phase B3**: the **interactive terminal** — a real PTY into the user's Cabin over a **bidirectional WS**, shown via `xterm.js`. The largest attack surface in the epic (interactive arbitrary execution), so it got its own security review — but everything runs INSIDE the already-isolated gVisor Cabin (kernel-filtered syscalls, deny-by-default egress, CPU/mem quotas, bounded by the Cabin's hard max lifetime + idle reaper; no host access). The Cabin contract gained a PTY capability: `CabinTerminal` ABC (`read()→bytes`/`write(bytes)`/`resize(cols,rows)`/`close()`) + `CabinBackend.open_terminal(user_id, *, cols=80, rows=24)`. `NullCabinBackend` (CI default) is a deterministic queue-backed echo terminal (canned prompt on open, echoes input) so the full WS bridge is testable with NO container; `GVisorCabinBackend` (opt-in, **lazy `aiodocker`**) opens a real PTY exec (`/bin/sh -i`, `Tty=True` + `AttachStdin=True`; `CabinError("BACKEND_UNAVAILABLE")` if absent). `CabinService.open_terminal(user_id, session, *, cols, rows)` ensures the Cabin then delegates. New `app/api/v1/terminal.py` — `WS /api/v1/voyages/{id}/terminal` — reuses `observation_deck`'s `_ws_credentials`/`_authenticate_ws_token`/`_load_authorized_voyage` + close codes: **owner-scoped** (close `1003` if the voyage isn't owned), **authenticated** via the `grandline-bearer` subprotocol (close `1008` on bad/missing token or when `terminal_enabled=False`), then it races two pump tasks (`{"type":"output","data"}` server→client; `{"type":"input"}`→`write`, `{"type":"resize"}`→`resize` client→server; malformed/unknown frames ignored) with an idle timeout (`terminal_idle_timeout_seconds`, default 600s → close `4408`), tearing both down + `terminal.close()` in a `finally`. No secret is logged; PTY bytes pass through verbatim. New settings `terminal_enabled=True`, `terminal_idle_timeout_seconds=600`. No migration; `aiodocker` stays lazy (app imports without Docker). Frontend: new deps `@xterm/xterm` + `@xterm/addon-fit`; `hooks/useCabinTerminal.ts` (WS lifecycle, subprotocol auth, clean teardown) + `components/observation-deck/TerminalPanel.tsx` (xterm Terminal + FitAddon + ResizeObserver), mounted as a **Terminal** sub-section (App/Terminal toggle) in the **Preview** deck view. **This COMPLETES the deck-capabilities epic** (A1–A3, B0–B3, C0–C2) — code review, the live app + interactive terminal, and per-user Dial config, all on the per-user Cabin + Sea Chest substrate.
