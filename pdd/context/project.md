...
**Frontend** (Observation Deck, Next.js App Router + React + Tailwind + Zustand + TanStack Query): the deck is implemented — Sea Chart / Crew Map / Ship's Log views, a multi-voyage event store with replay→live cutover and LRU buffering, WS/SSE streaming, playback, intervention controls, command palette + keyboard shortcuts, and (Phase 24) the Fleet Switcher with lossless per-voyage deck state.
